...
def mount_personality_routes(
    ...
    *,
    ...
    async def _save(request: Request) -> dict:
        ...
        if not name_s:
            return JSONResponse({"ok": False, "error": "invalid_name"}, status_code=400)  # type: ignore[return-value]
        try:
            logger.info(
            ...
            return JSONResponse({"ok": False, "error": str(e)}, status_code=500)  # type: ignore[return-value]
    ...
    async def _save_raw_get(name: str, instructions: str = "", tools_text: str = "", voice: str = "cedar") -> dict:
        name_s = _sanitize_name(name)
        if not name_s:
            return JSONResponse({"ok": False, "error": "invalid_name"}, status_code=400)  # type: ignore[return-value]
        try:
    ...
    async def _apply(
        ...
        loop = get_loop()
        if loop is None:
            return JSONResponse({"ok": False, "error": "loop_unavailable"}, status_code=503)  # type: ignore[return-value]
